...
def train_vectors(iteration=10):
    w2v = Word2Vec(log_filename="./Data/hashtag_corpus.txt", output_filename="./Data/wordVectors.txt", embedding_dimension = 40, half_window_size=5, iteration=iteration)
    w2v.train()
def hyper_search():
    ...
    for emb_size in embedding_size:
        for window in window_size:
            w2v = Word2Vec(log_filename="./Data/hashtag_corpus.txt", output_filename=out_path+str(emb_size)+str(window)+"wordVectors.txt", embedding_dimension = emb_size, iteration=10, half_window_size=window)
            [iList, lossList, accuracy_i_list, accuracy_list, f1_list] = w2v.train()
            plt.subplot(len(embedding_size), len(window_size), index)
            plt.plot(accuracy_i_list, f1_list, label="f1")
            plt.plot(accuracy_i_list, accuracy_list, label="f1")
            plt.ylabel(str(emb_size)+"_"+str(window))
            index = index + 1
    plt.show()
...
def training(has_dict, iteration=10):
    if not has_dict:
        gen_corpus()
        train_vectors(iteration=iteration)
        generate_dict_of_hashtag()
